...
async function enviarAlertaMake(data) {
  if (!MAKE_WEBHOOK_URL) {
  ...
  try {
    ...
    console.log("✅ Alerta enviada a Make correctamente.");
  } catch (error) {
    console.error("❌ Error enviando alerta a Make:", error.message);
  }
}
// ====================================================
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const nuevo_stock = tipo_movimiento === 'entrada'
      ? stock_actual + qty
      : stock_actual - qty;
    ...
    if (nuevo_stock <= stock_minimo_seguro) {
      ...
      const tipoAlerta = nuevo_stock === 0
        ? 'stock_cero'
        : 'stock_bajo';

      const mensaje = nuevo_stock === 0
        ? 'Producto agotado.'
        : 'Nivel de reorden alcanzado.';
      ...
      // ====================================================
      // 🚀 ENVÍO AUTOMÁTICO A MAKE
      // ====================================================

      await enviarAlertaMake({
        evento: "STOCK_BAJO",
        ...
        mensaje: mensaje,
        prioridad: nuevo_stock === 0 ? "CRITICA" : "MEDIA",
        fecha: new Date()
      });
    ...
    res.json({
      ok: true,
      message: "Movimiento y alertas registrados correctamente"
    });
